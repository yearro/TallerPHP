...
class conex {
	...
	function __construct(){
		...
		$this -> conexion = @mssql_connect($config["Base_Datos"]["servidor"],$config["Base_Datos"]["usuario"],$config["Base_Datos"]["password"]) or $this -> pinta_error('No se estableci&oacute; la conexi&oacute; con el servidor.');
		if(!@mssql_select_db($config["Base_Datos"]["base_datos"],$this -> conexion))
			$this -> pinta_error('No se encontro la base de datos.');
	}
	function consulta($consul){
		$this -> resul_consul = mssql_query($consul);
		if(!$this -> resul_consul)
			$this -> pinta_error('No se pudo realizar la consulta.');
	}
	
	function extrae_registro(){
		if ($fila = @mssql_fetch_array($this -> resul_consul)){
			return $fila;
		} else {
			return false;
		}
	...
	{
		return @mssql_num_rows($this -> resul_consul);
	}
	
	function filas_afectadas()
	{
		return @mssql_rows_affected($this -> conexion);
	}
	function __destruct(){
